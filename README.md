...
| Name | Description | Input Registers | Output Registers | Flags | Preserves Stack | Last X |
|------|-------------|-----------------|------------------|-------|-----------------|--------|
...
| `?PRIME` | Tests `X` for primality and displays result | `X` | - | 00 set if prime | ✅ | A factor such that $Last X \mid X $ |
...
| `→a/b` | Convert the decimal number in `X` to a fraction stored as $\frac{Y}{X}$ | `X` | `X`, `Y` | 00 set if fraction found,<br/> 01 used for recursion | ❌ | Overwritten |
